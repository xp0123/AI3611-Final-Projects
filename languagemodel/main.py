...
def train():
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            writer.add_scalar('train_PPL', math.exp(cur_loss), (epoch-1) * (len(train_data) // args.bptt) + i)
            writer.add_scalar('train_loss', cur_loss, (epoch-1) * (len(train_data) // args.bptt) + i)
            total_loss = 0
            start_time = time.time()
        if args.dry_run:
            break
...
try:
    for epoch in range(1, args.epochs+1):
        ...
        writer.add_scalar('val_PPL', math.exp(test_loss), epoch)
