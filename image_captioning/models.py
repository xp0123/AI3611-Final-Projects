...
class DecoderWithAttention(nn.Module):
    ...
    def decay(self, epsilon, i, decay_method='none'):
        if decay_method == 'linear':
            k = 1
            c = 0.05
            return max(0.1, k - c*i)
        elif decay_method == 'exp':
            k = 0.95
            return epsilon * k
        elif decay_method == 'inv_sigmoid':
            k = 9
            return k / (k + torch.exp(torch.tensor(i / k)))
        else:
            return epsilon

    def forward(self, encoder_out, encoded_captions, caption_lengths, decay_method='none'):
        """
        Forward propagation.
        ...
        for t in range(max(decode_lengths)):
            ...
            if t == 0 or torch.rand(1) < epsilon:
                embeds = embeddings[:batch_size_t, t, :]
            else:
                # print("!!!!!!!! decay = ", epsilon)
                embeds = self.embedding(prev_timestamp_words)[:batch_size_t, 0]
            epsilon = self.decay(epsilon, t, decay_method)
...
class Captioner(nn.Module):
    def __init__(self, encoded_image_size, attention_dim, embed_dim,
        decoder_dim, vocab_size, encoder_dim=2048, dropout=0.5, decay_method='none', **kwargs):
        super().__init__()
    ...
    def forward(self, images, encoded_captions, caption_lengths):
        ...
        decoder_out = self.decoder(encoder_out, encoded_captions,
                                   caption_lengths.unsqueeze(1), self.decay_method)
        return decoder_out
