...
def print_eval_metrics(img_cap_dict, encoding_dict, model, word2idx,
        idx2word, images, max_len):
    print('\t\tGreedy:            ',
          get_bleu_score(img_cap_dict, greedy_predictions_gen(
            encoding_dict=encoding_dict, model=model, word2idx=word2idx,
            idx2word=idx2word, images=images, max_len=max_len)))

    for k in [3, 5, 7]:
        print(f'\t\tBeam Search k={k}:', get_bleu_score(
            img_cap_dict, beam_search_predictions_gen(beam_index=k,
                encoding_dict=encoding_dict, model=model, word2idx=word2idx,
                idx2word=idx2word, images=images, max_len=max_len)))
